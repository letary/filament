...
| 0044 | TAA | `TemporalAntiAliasingOptions::nearLimit` (world units, 0 = off): fragments nearer than it take their history at the SAME screen position instead of the camera's reprojection. TAA reprojects by the camera alone (no per-object motion vectors), so a first-person viewmodel — fixed on screen, moving through the world — trailed on every turn; the band keeps it clean (the FPS way: a viewmodel outside the reprojection). The pass converts the limit to the depth the buffer holds under this frame's projection (FrameHistory `taa.nearLimit` = the edge depth + which way "nearer" goes, so reversed Z needs no special case); `taa.mat` skips the reprojection inside it |
