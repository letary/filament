...
| 0035 | filament | `RenderableManager::setShadowReceiverOnly(instance, bool)` / `isShadowReceiverOnly`: with VSM / PCSS every shadow RECEIVER is drawn into the shadow maps whatever `setCastShadows()` says (`View.cpp`: `hasVSM() \|\| hasPCSS()` -> `Scene.cpp`); a renderable marked here stays out (a free bit of `Visibility`, off by default). creator-gl marks the baked statics while a light grid carries their shadow: in the map they shadowed the baked surfaces a second time - the sun taken out of everything under them, the light a transmitting cloth let through included |
